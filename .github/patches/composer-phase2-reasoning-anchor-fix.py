...
def patch_popover_test() -> None:
    path = Path('test/features/home/composer/composer_reasoning_popover_test.dart')
    text = path.read_text(encoding='utf-8')
    text = replace_once(
        text,
        """  Widget build(BuildContext context) {\n    final anchorKey = GlobalKey();\n    return MaterialApp(\n""",
        """  Widget build(BuildContext context) {\n    final anchorKey = GlobalKey();\n    final anchorLink = LayerLink();\n    return MaterialApp(\n""",
        'popover_test/add layer link',
    )
    text = replace_once(
        text,
        """              child: IconButton(\n                key: anchorKey,\n                icon: const Icon(Icons.psychology),\n                onPressed: () => showComposerReasoningPopover(\n                  pageContext,\n                  anchorKey: anchorKey,\n                  currentBudget: -1,\n""",
        """              child: CompositedTransformTarget(\n                link: anchorLink,\n                child: IconButton(\n                  key: anchorKey,\n                  icon: const Icon(Icons.psychology),\n                  onPressed: () {\n                    final renderObject =\n                        anchorKey.currentContext?.findRenderObject();\n                    if (renderObject is! RenderBox || !renderObject.hasSize) {\n                      return;\n                    }\n                    final anchorRect = renderObject.localToGlobal(Offset.zero) &\n                        renderObject.size;\n                    showComposerReasoningPopover(\n                      pageContext,\n                      anchorLink: anchorLink,\n                      anchorRect: anchorRect,\n                      currentBudget: -1,\n""",
        'popover_test/use composited target',
    )
    text = replace_once(
        text,
        """                  onBudgetChanged: onBudgetChanged,\n                  onModelChanged: onModelChanged,\n                ),\n              ),\n""",
        """                      onBudgetChanged: onBudgetChanged,\n                      onModelChanged: onModelChanged,\n                    );\n                  },\n                ),\n              ),\n""",
        'popover_test/close target callback',
    )
    path.write_text(text, encoding='utf-8')
