...
// Adapted from the OpenMOSS/MOSS-TTS-Nano Android ONNX Runtime example.
// Source: https://github.com/OpenMOSS/MOSS-TTS-Nano/tree/main/examples/android_onnx_runtime
...
class MossOnnxEngine(
        ...
        OnnxTensor.createTensor(
            ...
            OnnxTensor.createTensor(
                ...
                outputs.use {
                    ...
                    val length = min(
                        reportedLength,
                        channels.minOfOrNull { channel -> channel.size } ?: 0,
                    )
                    return FloatArray(length) { sampleIndex ->
                        channels.sumOf { channel -> channel[sampleIndex].toDouble() }.toFloat() /
                            channels.size
                    }
                }
            }
    ...
    companion object {
        ...
        private fun OnnxTensor.scalarInt(): Int {
            return flattenIntTensorValue(value).firstOrNull()
                ?: error("Scalar int tensor is empty")
        }
    ...
    companion object {
        fun fromJson(json: JSONObject): ModelManifest {
            return ModelManifest(
                ...
                generationDefaults = GenerationDefaults.fromJson(
                    json.optJSONObject("generation_defaults"),
                ),
                builtinVoices = json.optJSONArray("builtin_voices")?.let { voices ->
                    List(voices.length()) { index ->
                        BuiltinVoice.fromJson(voices.getJSONObject(index))
                    }
                } ?: emptyList(),
            )
        }
    ...
    companion object {
        fun fromJson(json: JSONObject): PromptTemplates {
            return PromptTemplates(
                userPromptPrefixTokenIds =
                    json.getJSONArray("user_prompt_prefix_token_ids").toIntArrayCompat(),
